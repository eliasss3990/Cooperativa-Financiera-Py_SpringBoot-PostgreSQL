...
dev-stop-and-delete-volumes: ## Detiene los contenedores y borra los volumenes
	$(COMPOSE_DEV) down -v

.PHONY: dev-restart-backend
dev-restart-backend: ## Reinicia unicamente el contenedor del backend
...
dev-show-logs-all: ## Muestra los logs de todos los servicios en tiempo real
	$(COMPOSE_DEV) logs -f

.PHONY: dev-status-containers
dev-status-containers: ## Muestra que contenedores estan corriendo actualmente
	$(COMPOSE_DEV) ps
...
db-open-psql-console: ## Abre la consola de PostgreSQL dentro del contenedor
	$(COMPOSE_DEV) exec -e PGPASSWORD="$(POSTGRES_PASSWORD)" postgres-db \
		psql -U $(POSTGRES_USER) -d $(POSTGRES_DB)

.PHONY: db-show-logs
db-show-logs: ## Muestra los logs especificos de la base de datos
	$(COMPOSE_DEV) logs -f postgres-db

.PHONY: db-snapshot-save
db-snapshot-save: ## Guarda un snapshot de la base de datos dev
	@bash scripts/db-snapshot.sh save

.PHONY: db-snapshot-restore
db-snapshot-restore: ## Restaura la base de datos desde un snapshot
	@bash scripts/db-snapshot.sh restore

.PHONY: db-snapshot-list
db-snapshot-list: ## Lista todos los snapshots guardados
	@bash scripts/db-snapshot.sh list

.PHONY: db-snapshot-delete
db-snapshot-delete: ## Elimina snapshots guardados
	@bash scripts/db-snapshot.sh delete

##@ Calidad de Codigo

.PHONY: qa-run-all-checks
qa-run-all-checks: ## Compila, pasa linters y ejecuta los tests
...
qa-run-tests-only: ## Ejecuta unicamente los tests unitarios
...
env-reset-full: ## Borra todo, reconstruye imagenes y levanta el entorno desde cero
	@bash scripts/reset-dev.sh

.PHONY: env-reset-database-only
env-reset-database-only: ## Borra la base de datos, la inicializa limpia y reinicia el backend
